...
def card_value(card):
    ...
    The funciton also makes the ace value 11. We'll later adjust it so that it is worth 1 when exceeding 21, even though this is only a simplification of the game.
...
def calculate_total(hand):
    ...
    However, this is a limitation of our model; what it does in fact is transforming aces into a value 1 if the total is over 21.
    In reality, in BlackJack if a player stays with an ace it should remain an 11.
    But if the player hits an ace it should become a 1, and all the next ones too.
    (This would not apply to a blackjack hand which would have the ace worth 11 even though its a second card if the second drawn card is J,Q or K).
    This limitation exists in our model since we do not handle dynamic ace adjustments based on whether the player stays or hits but based on the total value of the hand.
